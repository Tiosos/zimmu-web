...
describe('the open cabinet', () => {
  ...
  // App is the only place carcaseHardware is called, and it hands the result to BomModal as a
  // prop. tsc proves the shape and nothing else — passing `[]`, or forgetting the scene changed,
  // typechecks perfectly and shows an empty Hardware tab. Only a test that renders App sees it.
  //
  // The scene is built through `regenerateComponents` rather than from `partsOfBase600`: that
  // fixture carries boards with no cuts, and hardware is counted off cuts, so it would prove
  // nothing but that an empty list renders empty.
  it('lists the hardware the open cabinet generated in the Hardware BOM tab', async () => {
    mockScene = regenerateComponents({
      parts: [],
      materials: { ...PRESET_MATERIALS },
      hardware: [],
      joints: [],
    ...
    fireEvent.click(await screen.findByRole('tab', { name: 'Hardware' }))

    const bomPanel = screen.getByTestId('bom-panel')
    expect(within(bomPanel).getByText('Generated by the cabinets')).toBeTruthy()
    // A catalogue name, not just the cabinet's label: the label also appears on the Boards tab's
    // rows, so it would not prove the hardware pass ran.
    expect(within(bomPanel).getByText('110° hinge c/w plate')).toBeTruthy()
    expect(within(bomPanel).getAllByText(cabinet.label).length).toBeGreaterThan(0)
  })
