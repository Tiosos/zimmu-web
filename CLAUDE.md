...
- **The regeneration pipeline order is fixed.** Three stages, always, via `applyPipeline` in `useScene.ts`: `regenerateDrawers(scene)`, then `regenerateComponents(scene)`, then `reconcileJoints(scene)`. Drawers lead because the dependency runs one way — a drawer reads nothing a carcase emits (the section tree, its own parameters, the materials), while the carcase's slide machining is a figure about the box standing in the opening, so the other order would machine the cabinet against a box it had not built yet. Carcases then emit parts and joints; `reconcileJoints` derives cuts and seats from those joints. All three stages are pure and idempotent.
