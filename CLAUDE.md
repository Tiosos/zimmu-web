...
│   │                    overlay front, face frame and toe kick included. Material slots only,
│   │                    never overrides
...
│   │                    a dangling or cyclic anchor and freezes that cabinet where it was
│   ├── runs.ts          runsOf(components, materials) → Run[] — maximal chains joined by left/right
│   │                    anchors, undirected, in geometric order. A front/back anchor starts a new run
│   ├── dragAnchor.ts    SNAP_MM + anchorForDrop — what anchor would put the cabinet here. Inverts
│   │                    anchoredPosition, reading anchor.ts's own face tables. Pure, no DOM.
│   │                    Also reports the target's rotation as a UI default
...
│   │                    swinging around its own front-left-bottom corner
│   ├── blindCorner.ts   BLIND_CLEARANCE + blindWidthOf + cornerWarnings — the blind portion must be
│   │                    at least as wide as the run returning into it is deep
...
│   │                    Also frontGeometryOf (what every front is measured against) and
│   │                    faceFrameContactPairs
│   ├── faceFrame.ts     faceFrameGeometry(root, outer, frame) — stiles, rails and each leaf's
│   │                    opening inside the frame, or null where it declines. The whole frame rule,
│   │                    stated once and read by three consumers, none of which reads another
...
│   │                    reveal rule, for all three mounts, framed or not
...
│   ├── regenerateDrawers.ts  Pure Scene → Scene; one of two passes that add or remove components.
│   │                    Reconciles one drawer per drawer-front opening, then emits its five boards
│   ├── regenerateFaceFrames.ts  Pure Scene → Scene; the other. One frame per framed cabinet,
│   │                    reconciled by cabinet, whose boards are the stiles and rails
│   ├── reconcileBoards.ts  reconcileBoards + GeneratedBoard — a component's boards against what it
│   │                    now wants, by role key. Shared by the drawer and frame passes so the
│   │                    detached-board preservation rules are stated once
...
│   ├── PlanView.tsx     The whole job from above as interactive SVG: a footprint per cabinet, a
│   │                    dashed outline per run, drag to move or to anchor. The second interactive
│   │                    SVG, following SectionElevation's precedent
...
- **`ZimmuFile` serialization** rounds floats to 6 decimal places. The current file format version is `FILE_FORMAT_VERSION = 20` in `useFile.ts` (v11 added the component tree; v12 added the carcase `backSetback` parameter; v13 replaced `dividers` and `fixedShelves` with the `section` tree; v14 moved panel thickness onto the material; v15 added the `screw` joint kind; v16 moved `adjustableShelves` off the cabinet onto each section's `interior`, dropping `startHeight`; v17 added `frontMaterial`/`frontMount`/`frontReveal`, `Section.front` and `interior.fixedShelves`; v18 added the drawer component; v19 added the carcase anchor; v20 added the face frame — `frame`, `frameMaterial`, `'half-overlay'` and the `faceFrame` component).
...
- **A cabinet's box is what it occupies, not its shell.** `carcaseBounds` is the one statement of it, and four things fall outside the shell: an applied back reaches behind `y = D`, a face frame and any door that is not inset in front of `y = 0` — half-overlay protrudes as far as overlay, and a rule asking `=== 'overlay'` counted it as nothing — and a toe kick fills `z ∈ [0, toeKickHeight]` below the carcase floor. A frame counts only where it was built, read through `frontGeometryOf`, so a declined frame occupies nothing. Anchoring on the shell floats every toe-kick cabinet its own kick height off the ground — which is what settled the question. It reads the cabinet's material **slots** only, never a part's overrides: overrides live on emitted parts, and placement runs before they exist, so reading them would make the pass a fixed-point iteration.
- **An anchor derives position; rotation is always the user's.** A corner is a cabinet that is both turned *and* anchored, and those are two separate facts. The meeting face is the one whose outward normal most opposes the target face's — realised as AABB adjacency, which needs no special case for a turned cabinet. The tempting rule, "the opposite face meets it", is wrong and **every straight-run fixture passes it**: only a corner fixture catches it, because the return cabinet's back is a plane of constant world x while the target's front is a plane of constant world y.
- **`anchor === undefined` is the detached state.** No `driven` flag exists on a carcase: presence or absence already draws the line `driven: false` draws for a part. A dangling or cyclic anchor is detached by `resolvePlacement`, which **freezes the cabinet where it last resolved** — snapping it to the origin loses it, and the dropped anchor is unrecoverable.
- **Only cycle *members* are detached, never bystanders.** Anchors form a functional graph (out-degree ≤ 1), so a walk that halts on "revisits any node I have seen" halts for every transitive ancestor of a cycle too. Detach a cabinet whose chain returns to **itself**; a chain that runs into someone else's cycle keeps its anchor, because that cycle's own members are detached on their own turn and `positionOf` then freezes their position for anyone anchored to them. The earlier rule — *detach every member of a chain that revisits an id* — shipped as a Critical bug that silently destroyed a legitimate anchor three cabinets away from the cycle.
- **A run is a straight line, so only a left or right anchor continues one.** `runsOf` builds an **undirected** graph over honoured left/right links — a cabinet on each side of one target is one run of three, because direction says who moves when the target moves, not what is in the line — and orders members **geometrically** (`x0`, then `y0`), the same choice `sectionOpenings` makes, so a run assembled from either end reads the same. A front or back anchor **starts a new run**, which is what leaves a corner as two runs to compare rather than one L-shaped one. An anchor is honoured on exactly `resolvePlacement`'s terms, and testing that guard means calling `runsOf` on **raw components**: routing the fixture through the pipeline lets `resolvePlacement` detach the cross-frame anchor first, and the clause survived its mutation until a test stopped resolving first.
- **A cabinet's world box is stated once.** `worldBoundsOf` composes `carcaseBounds` → `rotatedBounds` → `translatedBounds`; `localRotatedBoundsOf` is the same box before translation, which is what `anchoredPosition` wants for the cabinet being placed. `resolvePlacement` must translate by the position it is **computing**, never by `target.position` — stale until that write happens — so it calls the local form, not `worldBoundsOf`. Rotations are stored in **degrees** (`composeWorldMatrix` multiplies by `DEG2RAD`); a test written in radians turns a cabinet 1.57° and passes for the wrong reason.
- **World y runs back and SVG y runs down, and that flip happens in one function.** `toSvg` in `PlanView.tsx` is the only place `yMax - y1` is written — the same rule as `SectionElevation`'s `H - z1`, for the same reason. It must read the **far** edge: using `y0` gives every front-flush cabinet the same screen y, and a job flipped front-to-back looks entirely plausible until one cabinet is deeper than its neighbour, which is why the test that pins it compares a 560-deep cabinet against a 500-deep one.
- **A drop answers in anchor terms, and the arithmetic is not in the handler.** `anchorForDrop` inverts `anchoredPosition` at `gap = 0`, reading `anchor.ts`'s exported `FACE_NORMAL` and `IN_PLANE` rather than restating them — a second copy is how a drop comes to name a different face than the one the resolver honours. It is pure millimetres, so it is tested against real figures rather than through a gesture happy-dom cannot measure: happy-dom reports a **zero-sized** `getBoundingClientRect`, so any screen-to-world arithmetic in a handler is untestable there, and `PlanView`'s own drag tests pin a rect and wrap each phase in `act` (the listeners are attached by an effect, so an unflushed `pointerdown` leaves the `pointermove` landing on nothing). Candidates rank by distance on the **face's normal axis alone**: the in-plane axes are where the user is free to slide. Offsets snap to flush within `SNAP_MM` and otherwise keep what was dragged. A test expressing its figures in `SNAP_MM` moves with the constant and cannot catch it shrinking — one absolute figure is what pins that.
- **A turn is a rotation plus a translation.** A component rotates about its **local origin**, and a cabinet's origin is its front-left-bottom corner, so changing `rotation.z` alone swings a 600 × 578 base unit most of its own size across the room. `turnedInPlace` states the compensation once and preserves the **footprint centre**, which is what a plan-view turn looks like to a person. An anchored cabinet has its position rewritten by `resolvePlacement` anyway, so the free-placed one is the case that needs it. The turn wraps at 360°: a stored rotation of 3600 is the same cabinet and a worse thing to read in a file.
- **Only a front anchor makes a corner, and the blind portion is what the return run eats.** A **blind leaf** is one wearing no door — `front === undefined`, `'panel'` or `'false-front'`; `'door'` and `'drawer-front'` are the accessible part. `cornerWarnings` requires a target's blind leaves to be at least as wide as the anchored cabinet is **deep**, read off that cabinet's own `carcaseBounds` and never its world footprint — which is the same number seen through whatever rotation put it there, so reading it would make the rule depend on how the corner happens to be turned. A fixture whose blind leaf is a bare opening exercises one branch of the blind test only: dropping `'panel'` from the rule killed nothing until a panelled and a false-fronted leaf each got a case. `BLIND_CLEARANCE` is **0** — the spec's rule exactly — and is isolated because whether bare contact is enough is a woodworker's call nobody has made, not a figure anyone has checked.
- **A drop suggests a rotation; it never sets one.** `anchorForDrop` reports the target's `rotationZ` as a UI default and keeps it out of `Anchor` entirely: an anchor derives **position** only, and a corner is a cabinet both turned *and* anchored — two facts, not one. Without the suggestion the corner gesture does not work at all: dropping onto a blind unit's front face leaves the cabinet at 0°, facing out of the wall rather than along it.
- **Every placement figure a drag produces is a whole millimetre.** Both dragging surfaces work in continuous pixels, so without `roundMm` a free drop lands on something like `225.43806578321403` — **measured in the running app**, not invented. The rule sits in `dragAnchor.ts`, applied to the free position *and* to a past-the-snap offset, so the plan view and the gizmo cannot round differently and neither half is left raw. It composes with the face snap rather than replacing it: the grid rounds the figure, `SNAP_MM` still pulls a near-flush edge to flush.
- **Two cabinets standing side by side leave exactly one reveal between their doors.** The third clause of `frontReveal`'s own documentation, and the one that had no test: it was unreachable until placement could butt two carcases together, which is why the placement design named it as part of its justification. **Measured**, not derived — a 600-wide cabinet at reveal 3 emits its door 597 wide at local `x = 1.5`, so an overlay cell reaches the cabinet's **outer edge** at the cabinet boundary and gives back half a reveal there; it is the *midline* rule only where a cell meets another cell. Reasoning from the midline instead predicts a gap of `thickness + reveal` and is wrong. `adjacentFronts.test.ts` pins the gap at four reveal values so it tracks 1:1 rather than matching a constant at the preset's value.
- **A gizmo drag yields the drag to orbit, and only while a handle is held.** `TransformControls`'s `dragging-changed` sets `controls.enabled`, which is the narrowest possible suspension and the reason the gizmo needs no mode flag. Every other gesture in this app is **click**-based — snap, cut and the five joint tools are all discriminated from a drag by `handleClick`'s 4-pixel threshold — so `interactionActive` gates only the cursor and face routing and has never had to touch orbit. The gizmo is the first drag gesture, and the same `dragging` flag stops a drag that ends inside those 4 pixels from deselecting the cabinet it just moved.
- **A gizmo sits on the box, not on the origin.** A cabinet's `position` is its front-left-bottom corner, so handles placed there hang off the cabinet rather than on it. `gizmoCentreOf` and `positionForCentre` are inverses about the **occupied** box — the same centre `turnedInPlace` preserves. `positionForCentre` reads `localRotatedBoundsOf`, never `worldBoundsOf`: the translation is the very thing being solved for, and using the translated box makes the round trip drift by the cabinet's own position, which looks correct for a cabinet at the origin and for no other. The gizmo attaches to an invisible **proxy** `Object3D`, because the viewport renders parts and has no object for a component, and that proxy is never moved mid-drag — the scene re-renders on every committed drop, and yanking it back to the resolved centre while a handle is held fights the pointer.
- **A gizmo drop and a plan-view drop are the same rule.** `dropForCentre` converts centre → origin and hands off to `anchorForDrop`, adding no snapping arithmetic of its own, which is what makes a cabinet dragged in 3D land where the same cabinet dragged in plan would. It does **not** apply the drop's suggested `rotationZ`: that is a plan-view default for a corner drag, and a 3D drag that silently span the cabinet under the pointer would be a surprise. Because the gizmo offers **z**, and z is `anchorForDrop`'s in-plane `v` for a left/right/front/back face, a wall unit dragged beside another snaps flush in height through that same rule rather than one of its own.
- **Millimetres-per-pixel is the larger of the two axis ratios.** `preserveAspectRatio="xMidYMid meet"` picks the **smaller** scale so the whole viewBox fits, so the inverse is the larger ratio. Reading the x ratio alone drags at the wrong rate whenever the pane is the tall one.
- **The anchor graph is a second graph over the same components.** `wouldCycle` and `breakComponentCycles` guard `parentId`; they are the model for the anchor guard, never its implementation. An anchor may only name a cabinet with the same `parentId`, so both sit in one frame and no ancestor composition is needed — and `PlacementPanel` must not offer a target the pass would then detach.
- **The regeneration pipeline order is fixed, and it is five stages.** `resolvePlacement(scene)`, then `regenerateFaceFrames(scene)`, then `regenerateDrawers(scene)`, then `regenerateComponents(scene)`, then `reconcileJoints(scene)`, always, via `applyPipeline` in `useScene.ts`. Placement leads because `reconcileJoints` reads component position **transitively** — `deriveJoint` resolves each part's world matrix through its ancestors (`resolveWorldMatrix(housed, byId)` in `geom/dado.ts`), so a joint derived before its cabinet has moved is derived against the wrong world placement, and running placement last leaves the pipeline non-idempotent. A grep for `.position` shows none of this: the dependency runs through a matrix, and that grep is exactly what made an earlier draft of this file call the ordering a convention. Drawers lead because the carcase's slide machining reads drawer *parameters* while the drawer reads nothing the carcase *emits* — its inputs are the section tree, the drawer params and the materials, all of which exist before the carcase pass. Reversed, a new drawer front needs two passes to settle: the first bores no slide screws and the second does. The face frame leads the generators for the same reason: it reads nothing any of them emit. All five stages are pure and idempotent, and `regenerateFaceFrames` and `regenerateDrawers` are the only two that add or remove **components**.
...
- **`driven` on a component means what it means on a part.** Only `DrawerComponent` and `FaceFrameComponent` carry it. A detached drawer or frame is the user's: no regeneration, no deletion — and a board the user detached from a *driven* frame that is then dropped is re-homed on the cabinet rather than left naming a parent that no longer exists. Carcases and groups deliberately do not have it, because a detached carcase has no defined meaning.
- **`frame === undefined` is the frameless state, and a frame's geometry is stated once.** No `frameMode` exists, by the same rule as `anchor === undefined`. `faceFrameGeometry` takes the cabinet's front **rectangle**, never its parameters — so it carries no base-mode branch (a first draft of the plan tested `baseMode === 'ladder'`, which is the *side panel's* extent, and would have run every toe-kick cabinet's frame to the ground) and no import back into `carcaseRoles`, which would be a cycle. Stiles run full height and rails fit between them. Three consumers read it — the frame boards, the fronts, and the bounds — and none reads another's output: `drawerBoxMetrics`' shape reused.
- **`frontGeometryOf` is the one statement of what a front is measured against.** The validator, the box table, the machining and the frame pass all read it, and it carries the frame's openings **only where the frame resolved** — so a frame stage 1 declines leaves every front exactly where a frameless cabinet puts it, and the door and the frame boards cannot disagree about whether there is a frame to hang on. A full-overlay door needs no new rule on a frame (the frame's outer edge *is* the cabinet's) and stands forward by the frame's thickness; half-overlay is the midline rule an overlay front already follows at a division, applied to the frame's members, so two butted half-overlay cabinets show one stile plus one reveal; inset measures to the frame opening, flush with the frame's face.
- **A frame the stage cannot build is not an error.** A validation error refuses the **whole cabinet** — `carcaseBoxes` returns `[]` — so a framed divided cabinet or drawer front raised as an error would make the cabinet vanish when the user ticks "frame". Only the frame's own impossibilities are errors (no opening, a zero-width member, half-overlay with no frame); a stage limit is a note in the panel. For the same reason `onSetFrame` puts half-overlay back to overlay when the frame comes off.
- **A framed door is bored for no hinge until face-frame hinges exist.** A frameless cup-and-plate pattern would put its plates in a side the stile covers, and the hardware list counts hinges off those bores — so declining the bores is also what keeps a frameless hinge off the BOM. The rule a door too thin to bore already follows.
- **A frame declares its contacts, and they are honoured within one cabinet only.** Stile against rail (the spec's "internal contacts"), a member against a carcase panel's front edge (the mirror of an applied back on the rear edges), a front standing on the frame (the overlay front's contact, one surface out). `faceFrameContactPairs` reads them off the emitted boxes; the checklist honours a pair only when both boards resolve to the **same** cabinet, because every cabinet has a `left-side`. A framed Base 600 reads 10 / 10 exactly as a frameless one; before, it gained 12 open rows and 4 unresolved forever.
- **A frame's material is added when the user turns a frame on, never at load.** The migration must not touch a file's materials — the v14 test pins it — and seeding at load would add an unused material to every old file. `onSetFrame` adds it only when the scene has **no** material of that name, in the same undo step as the frame, so a user's own definition is never overwritten.
...
rule deliberately, watch the test fail, restore. Four rules earned the hard way:
...
- **Predict which tests should fail before running.** When fewer fail than expected, the gap is
  ...
  follow.
- **Confirm the unmutated run is green, then read *why* the mutated one failed.** A test that is
  already red for another reason — a missing import throwing a `TypeError` — makes every mutation
  look killed. Expect an `AssertionError` naming the rule; `Tests  no tests` is a broken mutation,
  not a survivor.
