...
test('the Sheets tab nests a real cabinet through the nest worker', async ({ page }) => {
  ...
  // A Base 600 emits 7 boards, all of material "18mm Ply" (src/scene/carcasePresets.ts).
  ...
  // `occupancyMask` → `nestSheets` at the default 14 mm clearance: one sheet, 72.83% used, nothing
...
test('stock too small for any panel reports every part as unplaced', async ({ page }) => {
  ...
  for (const label of ['Left Side', 'Right Side', 'Back', 'Bottom', 'Top', 'Shelf 1', 'Toe Kick']) {
